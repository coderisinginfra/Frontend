import React from 'react'
import ace from '../assets/parteners/ace.avif'
import bhutaniinfra from '../assets/parteners/bhutaninfra.avif'
import dlf from '../assets/parteners/dlf.avif'
import emar from '../assets/parteners/emar.avif'
import gaurs from '../assets/parteners/gaurs.avif'
import godrej from '../assets/parteners/godrej.avif'
import gulshan from '../assets/parteners/gulshan.avif'
import jaypee from '../assets/parteners/jaypee.avif'
import mahagun from '../assets/parteners/mahagun.avif'
import saviour from '../assets/parteners/savviour.avif'
import saya from '../assets/parteners/saya.avif'
import prateek from '../assets/parteners/prateek.avif'
import maxstates from '../assets/parteners/maxestates.avif'
import gygy from '../assets/parteners/gygy.avif'
import lodha from '../assets/parteners/lodha.avif'
import logix from '../assets/parteners/logix.avif'
import LT from '../assets/parteners/LT.avif'
import m3m from '../assets/parteners/m3m.avif'
import hiranandani from '../assets/parteners/hiranandani.avif'
import tata from '../assets/parteners/tata.avif'
import omaxe from '../assets/parteners/omaxe.avif'
import oberoi from '../assets/parteners/oberoi.avif'
import paras from '../assets/parteners/paras.avif'
import fairfox from '../assets/parteners/fairfox.jpeg'
import crc from '../assets/parteners/crc.jpg'


const Carousel = () => {
  return (
    <>
    <div className='crouseal-start-here'>
        <div className='flex-crouseal-images'>
            <img className="image-crouseal" src={ace} alt="ace" />
            <img className="image-crouseal" src={bhutaniinfra} alt="bhutani" />
            <img className="image-crouseal" src={gulshan} alt="gulshan" />
            <img className="image-crouseal" src={saya} alt="saya" />
            <img className="image-crouseal" src={gygy} alt="gygy" />
            <img className="image-crouseal" src={fairfox} alt="fairfox" />
            <img className="image-crouseal" src={crc} alt="crc" />
            <img className="image-crouseal" src={hiranandani} alt="hiranandani" />
            <img className="image-crouseal" src={omaxe} alt="omaxe" />
            <img className="image-crouseal" src={m3m} alt="m3m" />
            <img className="image-crouseal" src={LT} alt="lt" />
            <img className="image-crouseal" src={lodha} alt="lodha" /> 
            <img className="image-crouseal" src={oberoi} alt="oberoi" />
            <img className="image-crouseal" src={prateek} alt="prateek" /> 
            <img className="image-crouseal" src={maxstates} alt="maxstates" />
            <img className="image-crouseal" src={tata} alt="tata" />
            <img className="image-crouseal" src={jaypee} alt="jaypee" />
            <img className="image-crouseal" src={mahagun} alt="mahagun" />
            <img className="image-crouseal" src={logix} alt="logix" />
            <img className="image-crouseal" src={dlf} alt="dlf" /> 
            <img className="image-crouseal" src={emar} alt="emar" />
            <img className="image-crouseal" src={gaurs} alt="gaurs" />
            <img className="image-crouseal" src={godrej} alt="godrej" />
            <img className="image-crouseal" src={saviour} alt="saviour" />
            <img className="image-crouseal" src={paras} alt="paras" />
        </div>
        <div className='flex-crouseal-images'>
            <img className="image-crouseal" src={ace} alt="ace" />
            <img className="image-crouseal" src={bhutaniinfra} alt="bhutani" />
            <img className="image-crouseal" src={gulshan} alt="gulshan" />
            <img className="image-crouseal" src={saya} alt="saya" />
            <img className="image-crouseal" src={gygy} alt="gygy" />
            <img className="image-crouseal" src={fairfox} alt="fairfox" />
            <img className="image-crouseal" src={crc} alt="crc" />
            <img className="image-crouseal" src={hiranandani} alt="hiranandani" />
            <img className="image-crouseal" src={omaxe} alt="omaxe" />
            <img className="image-crouseal" src={m3m} alt="m3m" />
            <img className="image-crouseal" src={LT} alt="lt" />
            <img className="image-crouseal" src={lodha} alt="lodha" /> 
            <img className="image-crouseal" src={oberoi} alt="oberoi" />
            <img className="image-crouseal" src={prateek} alt="prateek" /> 
            <img className="image-crouseal" src={maxstates} alt="maxstates" />
            <img className="image-crouseal" src={tata} alt="tata" />
            <img className="image-crouseal" src={jaypee} alt="jaypee" />
            <img className="image-crouseal" src={mahagun} alt="mahagun" />
            <img className="image-crouseal" src={logix} alt="logix" />
            <img className="image-crouseal" src={dlf} alt="dlf" /> 
            <img className="image-crouseal" src={emar} alt="emar" />
            <img className="image-crouseal" src={gaurs} alt="gaurs" />
            <img className="image-crouseal" src={godrej} alt="godrej" />
            <img className="image-crouseal" src={saviour} alt="saviour" />
            <img className="image-crouseal" src={paras} alt="paras" />
        </div>
    </div>
    </>
  )
}

export default Carousel
